! Copyright (c) 2004-1015 Lars Nerger
!
...
!
!$Id: PDAF-D_enkf_alloc.F90 1561 2015-02-22 15:57:23Z lnerger $
!BOP
!
! !ROUTINE: PDAF_enkf_alloc --- PDAF-internal initialization of EnKF
!
! !INTERFACE:
SUBROUTINE PDAF_enkf_alloc(subtype, outflag)
...
! !USES:
  USE PDAF_memcounting, &
       ONLY: PDAF_memcount
  USE PDAF_mod_filter, &
       ONLY: screen, incremental, dim_ens, dim_p, forget, &
       rank_ana_enkf, type_forget, dim_bias_p,  &
       state, state_inc, eofU, eofV, sens, dim_lag
...
! ******************************
  ...
  on_filterpe: IF (filterpe) THEN
     ...
     ALLOCATE(eofU(1, 1), stat = allocstat) ! This is not used in EnKF
     IF (allocstat /= 0) THEN
     ...
     IF (allocstat /= 0) THEN
        WRITE (*,'(5x, a)') 'PDAF-ERROR(20): error in local allocation of eofV'
        outflag = 20
     END IF
     ! count allocated memory
  ...
  ELSE on_filterpe
     ...
     IF (COMM_couple /= MPI_COMM_NULL) THEN
        ...
        IF (screen > 2) WRITE (*,*) 'PDAF: enkf_alloc - allocate eofV of size ', &
