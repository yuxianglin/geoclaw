!$Id: prepoststep_ens_pdaf.F90 1555 2015-02-16 18:21:18Z lnerger $
!BOP
!
...
! !USES:
  ...
  USE mod_assimilation, &
       ONLY: dim_state, &
       incremental, filename, subtype, covartype
  ...
  notfirst: IF (.not. firsttime) THEN
     ...
     ELSE mype0c
        ...
        DO domain = 2, npes_filter
           ...
           ! Receive sub-arrays
           CALL MPI_recv(state(1+offset), nx_p*ny, MPI_DOUBLE_PRECISION, &
                domain - 1, 1, COMM_filter, MPIstatus, MPIerr)
